...
CRAWL FIX v3.1:
  - UA rotation across 6 realistic browser fingerprints
  - Full browser-like header sets (Accept, Sec-Fetch-*, Cache-Control, etc.)
  - 3-attempt retry with exponential back-off + jitter
  - Handles 403/429/503/Cloudflare redirects gracefully
  - SSL context fallback: verified -> unverified -> legacy cipher ctx
  - Compressed response support (gzip/brotli)
  - _block_reason + _crawl_note fields for front-end diagnostics
"""
...
import aiohttp, asyncio, os, json, re, ssl, random
...
# ═══════════════════════════════════════════════════════════════
# USER-AGENT POOL  —  6 real browser fingerprints rotated per attempt
# ═══════════════════════════════════════════════════════════════
...
def _make_headers(ua: str) -> dict:
    """Full realistic browser header set matched to the given UA."""
    ...
    headers = {
        ...
        "Accept-Encoding": "gzip, deflate, br" if not is_safari else "gzip, deflate",
        "Connection":      "keep-alive",
        "Cache-Control":   "no-cache",
        "Pragma":          "no-cache",
        "Upgrade-Insecure-Requests": "1",
    }
    # Sec-Fetch headers only make sense for Chromium-based browsers
...
def get_active_model() -> dict:
    ...
    if gpt_ok:
        return {"id": "openai/gpt-oss-20b", "label": "GPT-20 (OSS 20B)", "provider": "openrouter", "max_tok": 2000}
    return {"id": "llama-3.1-8b-instant", "label": "LLaMA 3.1 8B", "provider": "groq", "max_tok": 1800}
...
def detect_intent(url, title, h1, meta) -> str:
    ...
    scores = {
        "Transactional": sum(2 for s in ["buy","shop","price","order","purchase","cart","checkout","get started","sign up","subscribe","pricing","plan"] if s in combined),
...
def extract_keyword_intelligence(text: str, keyword: str) -> dict:
    ...
    return {"primary": keyword, "variations": variations[:12], "topic_clusters": topics[:8], "total_variation_count": len(variations)}
...
def compute_seo_score(data: dict, keyword: str) -> dict:
    ...
    if data.get("title_length",0)>70:   dims["on_page_signals"]-=2
...
# ═══════════════════════════════════════════════════════════════
# ROBUST FETCH  — the core fix for blocked competitors
# ═══════════════════════════════════════════════════════════════

# Codes that can be retried with a different UA / SSL strategy
_RETRYABLE_CODES = {403, 406, 429, 503, 530}
# Codes that are permanent — no point retrying
_PERMANENT_FAIL  = {404, 410, 451}
# Patterns in response body that indicate a bot challenge page
_BLOCK_PATTERNS  = [
    b"just a moment",       b"checking your browser",
    b"enable javascript",   b"cf-browser-verification",
    b"ddos-guard",          b"please wait",
    b"access denied",       b"bot detected",
    b"automated access",    b"robot or human",
    b"i'm not a robot",     b"captcha",
    b"security check",      b"human verification",
]

def _is_challenge_body(raw: bytes) -> bool:
    low = raw[:6000].lower()
    return any(p in low for p in _BLOCK_PATTERNS)

def _block_reason_msg(status: int, body: bytes) -> str:
    if status == 403:               return "Access denied (403) — server blocks automated requests"
    if status == 429:               return "Rate limited (429) — too many requests to this site"
    if status == 503:               return "Service unavailable (503) — likely Cloudflare or bot protection"
    if status == 530:               return "Cloudflare origin error (530)"
    if status == 404:               return "Page not found (404) — check the URL"
    if status == 410:               return "Page permanently removed (410)"
    if _is_challenge_body(body):    return "Bot/CAPTCHA challenge page — JavaScript render required"
    return f"HTTP {status} error"

async def _fetch_with_retry(url: str, max_attempts: int = 3) -> tuple:
    """
    Returns (status: int, body: bytes, block_reason: str|None).

    Strategy per attempt:
      Attempt 0 — Chrome UA, verified SSL, default headers
    ...
    try:
        ctx_legacy.set_ciphers("DEFAULT:@SECLEVEL=1")
    except Exception:
        pass
    ...
    for attempt in range(max_attempts):
        ...
        ssl_ctx = ssl_ctxs[attempt % len(ssl_ctxs)]

        # Progressive delay on retries to avoid hammering
        if attempt > 0:
            await asyncio.sleep(0.5 + random.uniform(0.3, 0.7) * attempt * 2)
        ...
        try:
            ...
            async with aiohttp.ClientSession(
                ...
                async with sess.get(
                    ...
                    if last_status in _PERMANENT_FAIL:
                        block_reason = _block_reason_msg(last_status, b"")
                        return last_status, b"", block_reason

                    # Read body
                    try:
                        last_body = await asyncio.wait_for(resp.read(), timeout=15)
                    except (asyncio.TimeoutError, Exception):
                        last_body = b""
                        block_reason = "Timed out reading response body"
                        continue

                    if last_status == 200:
                        if _is_challenge_body(last_body):
                            block_reason = _block_reason_msg(200, last_body)
                            continue  # try next UA
                        return 200, last_body, None  # SUCCESS

                    if last_status in _RETRYABLE_CODES:
                        block_reason = _block_reason_msg(last_status, last_body)
                        continue  # retry with different fingerprint

                    # Other 2xx/3xx that weren't followed
                    if last_status < 400:
            ...
            block_reason = f"SSL error (attempt {attempt+1}): {e}"
            continue
        except aiohttp.ClientConnectorError as e:
            block_reason = f"Connection failed: {e}"
            break  # DNS/network errors won't improve with retries
        except asyncio.TimeoutError:
            block_reason = f"Request timed out (attempt {attempt+1})"
            continue
        except Exception as e:
            block_reason = f"Fetch error: {type(e).__name__}: {e}"
            continue
...
async def extract_page(session, url: str, keyword: str, manual: dict) -> dict:
    ...
    result = {
        ...
        # Diagnostic fields shown on the front end
        "_block_reason": None,
        "_crawl_note":   None,
    }
 ...
 if status != 200 or not raw:
    ...
    try:
        ...
        result["seo_score"]           = sr["total"]
        result["seo_score_breakdown"] = sr["breakdown"]

        # Warn on suspiciously low word count (possible JS SPA)
        if result["word_count"] < 100:
            result["_crawl_note"] = (
                "Very little text extracted — this page may be JavaScript-rendered (React/Next.js/Vue SPA). "
                "Shown data is from static HTML only and may be incomplete."
            )
...
def compute_comparison(results: list, user_url: str, keyword: str) -> dict:
    ...
    comp_avg={k:avg(k) for k in ["word_count","h2_count","h3_count","keyword_count","internal_links","image_count","seo_score","keyword_density"]}
    gaps=[]
    ...
    if wc_diff<-400 and intent not in ("Transactional","Navigational"):
        ...
        gaps.append({"type":"content","severity":sev,"metric":"Content Depth",
            ...
            "message":f"Your content is {abs(int(wc_diff)):,} words shorter than the competitor average. For '{keyword}' ({intent} intent), deeper coverage signals authority to Google."})
    ...
    if not user.get("keyword_in_h1"):
        ...
        if kw_h1_cnt>=1:
            gaps.append({"type":"keyword","severity":"high","metric":"Keyword in H1 Tag",
                ...
                "message":f"Keyword missing from H1. Google gives significant weight to H1 alignment with the target query."})
    ...
    if kd<0.2:
        gaps.append({"type":"keyword","severity":"medium","metric":"Keyword Density",
            ...
            "message":f"Keyword appears too infrequently ({kd}%). Target 0.5–2% natural density."})
    ...
    if not user.get("keyword_in_meta"):
        ...
        if kw_meta_cnt>=len(comps)//2:
            gaps.append({"type":"keyword","severity":"medium","metric":"Keyword in Meta Description",
                ...
                "message":f"Keyword absent from meta description. Google bolds matching keywords in snippets — increasing CTR."})
    ...
    if h2_diff<-3:
        gaps.append({"type":"structure","severity":"medium","metric":"H2 Heading Structure",
            ...
            "message":f"Your page has {abs(int(h2_diff))} fewer H2 headings than competitors."})
    ...
    if user.get("h1_count",0)==0:
        gaps.append({"type":"structure","severity":"critical","metric":"Missing H1 Tag",
            ...
            "message":"No H1 tag detected. Google cannot determine page topic from structure."})

    if not user.get("canonical"):
        gaps.append({"type":"technical","severity":"medium","metric":"Missing Canonical Tag",
            "user_val":"Missing","comp_avg":"Expected",
            "message":"No canonical tag. Google may index duplicate versions, splitting ranking signals."})
    ...
    if user.get("canonical") and not user.get("canonical_match"):
        gaps.append({"type":"technical","severity":"high","metric":"Canonical URL Mismatch",
            "user_val":"External canonical","comp_avg":"Self-referencing",
            "message":"Canonical points to a different URL — actively harms this page's ranking ability."})
    ...
    if not user.get("schema_types"):
        ...
        if comp_schema>=1:
            gaps.append({"type":"technical","severity":"low","metric":"Structured Data (Schema)",
                "user_val":"None","comp_avg":f"{comp_schema}/{len(comps)} use schema",
                "message":f"{comp_schema} competitors use schema. Enables rich results and boosts CTR."})

    if not user.get("og_complete"):
        gaps.append({"type":"technical","severity":"low","metric":"Open Graph Tags Incomplete",
            "user_val":"Partial","comp_avg":"Full set (5 tags)",
            "message":"OG tags incomplete. Controls appearance on social platforms and referral traffic."})
    ...
    if il_diff<-10 and comp_avg["internal_links"]>5:
        gaps.append({"type":"structure","severity":"medium","metric":"Internal Link Count",
            ...
            "message":f"Your page has {abs(int(il_diff))} fewer internal links than competitors."})
    ...
    if img_diff<-4 and comp_avg["image_count"]>3:
        gaps.append({"type":"content","severity":"low","metric":"Visual Content",
            ...
            "message":f"Competitors average {int(comp_avg['image_count'])} images. Images improve dwell time."})
        ...
        if isinstance(mob,dict) and isinstance(mob.get("score"),(int,float)):
            if mob["score"]<50:
                gaps.append({"type":"speed","severity":"critical","metric":"Mobile Page Speed",
                    ...
                    "message":f"Mobile speed {mob['score']}/100 is critically low. Core Web Vitals are a confirmed ranking factor."})
            elif mob["score"]<70:
                gaps.append({"type":"speed","severity":"high","metric":"Mobile Page Speed",
                    "user_val":f"{mob['score']}/100","comp_avg":"Aim ≥ 90/100",
                    "message":f"Mobile speed {mob['score']}/100 needs improvement — affects bounce rate and rankings."})
    ...
    content_type_note=(f"⚠️ Format mismatch: your page is '{user_type}' but {comp_types.count(most_common)}/{len(comps)} competitors use '{most_common}'."
                       if user_type!=most_common and most_common!="Unknown" else None)

    comp_dates=[c.get("published_date") for c in comps if c.get("published_date") not in ("Not Found",None,"")]
    recent=[d for d in comp_dates if d>="2024"] if comp_dates else []
    freshness_note=(f"📅 {len(recent)}/{len(comps)} competitors have recent date-stamped content. Add datePublished schema."
                    if recent and user.get("published_date","Not Found")=="Not Found" else None)
    ...
    return {
        ...
        "ranking_probability":  compute_ranking_probability(user, comps, keyword),
        "user_url":             user_url,
...
async def call_ai(comparison: dict) -> dict:
    ...
    system_prompt = (
        "You are a 12+ year veteran SEO strategist. Think contextually — ranking is about satisfying "
        "search intent better than competitors. Give expert, specific, actionable advice. Explain WHY things "
        "matter for rankings. Be direct and practical. Always respond with valid JSON only — no markdown, no preamble."
    )
    user_prompt = f"""Analyse this SEO comparison and generate expert recommendations.

KEYWORD: "{keyword}" | INTENT: {intent} | FORMAT: {ctype}
USER SCORE: {comparison.get('user_score',0)}/100 | COMP AVG: {comparison.get('competitor_avg_score',0)}/100
RANKING PROBABILITY: {prob.get('probability',50)}% ({prob.get('label','Moderate')})

CRITICAL/HIGH GAPS:
{json.dumps([g for g in gaps if g['severity'] in ('critical','high')][:6], indent=2)}
MEDIUM: {json.dumps([g for g in gaps if g['severity']=='medium'][:4], indent=2)}
LOW:    {json.dumps([g for g in gaps if g['severity']=='low'][:3], indent=2)}

Return ONLY valid JSON:
{{
  "executive_summary": "2-3 sentences: current state, key issue, next action.",
  "ranking_rationale": "1-2 sentences WHY this page ranks or doesn't.",
            ...
            try:
                async with aiohttp.ClientSession() as sess:
                    async with sess.post(
                        ...
                        if resp.status == 200:
                            data    = await resp.json()
                            record_usage("openrouter", data.get("usage",{}).get("total_tokens",600))
        ...
        try:
            async with aiohttp.ClientSession() as sess:
                async with sess.post(
                    ...
                    if resp.status == 200:
                        data    = await resp.json()
                        record_usage("groq", data.get("usage",{}).get("total_tokens",500))
                        content = data["choices"][0]["message"]["content"].strip()
                        content = re.sub(r"^```(?:json)?\s*","",content)
                        content = re.sub(r"\s*```$","",content)
                        result  = json.loads(content)
...
def _fallback_ai(comparison: dict, model_label="AI Analysis") -> dict:
    gaps    = comparison.get("gaps",[])
    us, ca  = comparison.get("user_score",0), comparison.get("competitor_avg_score",0)
    keyword = comparison.get("keyword","")
    diff    = us - ca
    ...
    return {
        "executive_summary": f"Your page scores {us}/100, {abs(diff)} points {'ahead of' if diff>=0 else 'behind'} the competitor average ({ca}/100). {len(high)} critical issues need attention.",
        "ranking_rationale": f"Fixing the top {min(3,len(high))} gaps — especially keyword placement and technical signals — will most directly impact rankings for '{keyword}'.",
        ...
        "content_strategy": "Match the dominant content format of top-ranking pages and align depth with search intent.",
        "quick_wins": ["Add primary keyword to title tag","Set self-referencing canonical","Complete Open Graph tags"],
        "competitive_advantages": ["Run full analysis to identify strengths"],
        "content_gaps": ["Run content audit against top-3 ranking pages"],
        "_model": model_label,
    }
...
async def call_ai_chat(message: str, history: list, context: str, keyword: str) -> str:
    model_cfg = get_active_model()
    system = f"""You are an expert SEO strategist assistant inside SearchRNK Intelligence.
Use the analysis data below to give specific, accurate answers.

ANALYSIS CONTEXT:
{context}

RULES:
1. Only answer SEO, rankings, or digital marketing questions.
2. Reference actual numbers from the context.
3. Be direct and actionable.
4. Use **bold** for key points and bullet points for lists."""
...
async def health():
    ...
    return {
        "status":                 "ok",
        "service":                "SearchRNK Intelligence API v3.1",
